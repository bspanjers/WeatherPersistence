...
class QAR_temperature:
    ...
    def plot_fourier_fit(self, mid=False, conf_intervals=True, alpha=.1, fixed_params=False, plot=True):
        ...
        df_old, df_new = self.create_year_df(self.old.index.year[-1]), self.create_year_df(self.new.index.year[-1])
        if plot == True:
            plt.figure(dpi=300, figsize=(12, 8), facecolor="white")
        ...
        if plot == True: 
            ...
            if mid == True:
                ...
                plt.fill_between(lower_mid.index, lower_mid, upper_mid, color='green', alpha=0.1)
            
            if conf_intervals == True:    
                plt.fill_between(lower_new.index, lower_new, upper_new, color='red', alpha=0.1)
                plt.fill_between(lower_old.index, lower_old, upper_old, color='orange', alpha=0.1)
            plt.xlabel('Month')
    ...
    def plot_all_phi_coefs(self, iQuantiles=9, plot=True):
        ...
        if plot == True:
            plt.figure(figsize=(8, 3), dpi=100, facecolor="white") 
            ...
            plt.figure(figsize=(8, 3), dpi=400, facecolor="white")
    ...
    def bootstrap_one_path_original_len(self, df_params, vQuantiles, years=100):
        days = int(years * 365.25) - int(years/100)
        # Generate a date range for 300 years including leap years
        start_date = "1900-01-01"
        date_range = pd.date_range(start=start_date, periods=days, freq="D")
        date_range = date_range[~((date_range.month == 2) & (date_range.day == 29))]

        U_t = np.random.choice(vQuantiles, len(date_range) - 1)
        corresponding_columns = df_params.loc[:, [str(round(u, 2)) for u in U_t]].T.reset_index(drop=True)

        Ystar = np.zeros(len(U_t) + 1)
        Ystar[0] = 0.5#df.Temp.values[0]
        
        fourier_pers = self.create_fourier_terms(date_range, self.num_terms_pers, 'pers_').values
        fourier_const = self.create_fourier_terms(date_range, self.num_terms_level, 'pers_').values
        
        for i in np.arange(1, len(date_range)):
            Ystar[i] = np.dot(corresponding_columns.iloc[i-1, :-2 * self.num_terms_pers - 1], fourier_const[i-1, :].T) + np.dot(corresponding_columns.iloc[i-1, -(2 * self.num_terms_pers + 1):], fourier_pers[i-1, :] * Ystar[i-1])
        self.Ystar = Ystar
        #return Ystar
    ...
    def plot_unconditional_quantiles(self):
        plt.figure(figsize=(12,6), dpi=800, facecolor="white")
